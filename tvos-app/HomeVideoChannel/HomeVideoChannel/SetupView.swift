...
private enum SettingsTheme {
    ...
    static let accent = Color(red: 0.65, green: 0.91, blue: 0.87)
}
...
struct SetupView: View {
    ...
    @State private var preloadEnabled = true
    ...
    var body: some View {
        NavigationStack {
            ...
            .onAppear {
                if !hasLoadedConfig { loadFromConfig() }
                onRefreshStats?()
                hasLoadedConfig = true
                if focusedSetting == nil {
                    focusedSetting = isOnboarding ? .immichURL : .minDuration
                }
            }
            .onDisappear {
            ...
            .onChange(of: preloadEnabled) { _ in autoSaveSettingsIfNeeded() }
    ...
    // Keep cards mounted so validation can focus fields anywhere in the page.
    // SettingsCard erases each section's generic type to keep this layout small.
    ...
    private var settingsHelp: some View {
        HStack(spacing: 24) {
            QRCodeView(value: bananaSystemsGuideURL, size: 100)
    ...
    private var settingsHeader: some View {
        HStack(alignment: .center, spacing: 32) {
            ...
            Spacer()
            Button { save() } label: {
                Label(L10n.tr("common.done", "Done", comment: "Close settings action"), systemImage: "checkmark")
                    .font(.headline)
                    .frame(width: 220, height: 68)
                    .foregroundStyle(focusedSetting == .closeSettings ? Color.black : SettingsTheme.accent)
                    .background(focusedSetting == .closeSettings ? Color.white : Color.white.opacity(0.12),
                                in: RoundedRectangle(cornerRadius: 16))
            }
            .buttonStyle(.plain)
            .focused($focusedSetting, equals: .closeSettings)
            .accessibilityIdentifier("settings.done")
        }
    }

    private var connectionCard: SettingsCard {
        card {
    ...
    private var playbackCard: SettingsCard {
        card {
    ...
    private var transitionsCard: SettingsCard {
        card {
            VStack(alignment: .leading, spacing: 14) {
    ...
    private var feedbackCard: SettingsCard {
        card {
            VStack(alignment: .leading, spacing: 16) {
    ...
    private var cacheCard: SettingsCard {
        card {
            VStack(alignment: .leading, spacing: 14) {
    ...
    private var toolsCard: SettingsCard {
        card {
            VStack(alignment: .leading, spacing: 14) {
                ...
                booleanPicker(L10n.tr("settings.preload_enabled", "Preload Next Video", comment: "Diagnostic preload toggle"), isOn: $preloadEnabled, focus: .preloadEnabled)
                Text(L10n.tr("settings.preload_diagnostic_hint", "Turn off to compare buffering without next-video downloads. Automatic transitions then wait until the current video ends.", comment: "Diagnostic preload comparison explanation"))
                    .font(.caption)
                    .foregroundStyle(.white.opacity(0.65))
    ...
    private func loadFromConfig() {
        ...
        preloadEnabled = cfg.preloadEnabled
    ...
    private func save() {
        guard let next = buildConfig(requireCredentials: true, preserveExistingOnMissingCredentials: false) else {
            revealInvalidSetting()
            return
        }
        validationError = ""
        playbackError?.wrappedValue = ""
        if next != configStore.config { configStore.save(next) }
        dismiss()
    }

    private func revealInvalidSetting() {
        if !SettingsInputValidation.validServerURL(immichURL) || apiKey.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty {
            focusedSetting = !SettingsInputValidation.validServerURL(immichURL) ? .immichURL : .apiKey
        } else if !SettingsInputValidation.validNumber(minDuration, allowZero: true) || (Int(randomBatchSize) ?? 0) <= 0 {
            focusedSetting = !SettingsInputValidation.validNumber(minDuration, allowZero: true) ? .minDuration : .randomBatchSize
        } else if !SettingsInputValidation.validNumber(preloadSeconds, allowZero: false) || (Int(crossfadeDuration) ?? -1) < 0 || (Int(queueTarget) ?? 0) <= 0 {
            focusedSetting = !SettingsInputValidation.validNumber(preloadSeconds, allowZero: false) ? .preloadSeconds :
                ((Int(crossfadeDuration) ?? -1) < 0 ? .crossfadeDuration : .queueTarget)
        } else {
            focusedSetting = (Int(syncPageSize) ?? 0) <= 0 ? .syncPageSize : .syncMaxPages
        }
    }
    ...
    private func buildConfig(requireCredentials: Bool, preserveExistingOnMissingCredentials: Bool) -> AppConfig? {
        ...
        if !SettingsInputValidation.validServerURL(trimmedURL) {
            validationError = L10n.tr("errors.settings.invalid_server_url", "Enter a valid http:// or https:// server URL.", comment: "Invalid server URL")
            return nil
        }

        guard let minDurationValue = Double(minDuration), minDurationValue.isFinite, minDurationValue >= 0,
              let randomBatchValue = Int(randomBatchSize), randomBatchValue > 0,
              let preloadValue = Double(preloadSeconds), preloadValue.isFinite, preloadValue > 0,
        ...
        next.preloadEnabled = preloadEnabled
    ...
    @ViewBuilder
    private func card<Content: View>(@ViewBuilder content: () -> Content) -> SettingsCard {
        SettingsCard(content: content)
    }

    // Erase once per card, rather than carrying all of its row/modifier types
    // through the entire settings layout's generic metadata.
            ...
            content
                ...
                .background(
                    RoundedRectangle(cornerRadius: 28, style: .continuous)
                        .fill(Color(red: 0.075, green: 0.12, blue: 0.16))
                        .overlay(
                            RoundedRectangle(cornerRadius: 28, style: .continuous)
    ...
    private struct SettingsField: View {
        ...
        let focus: SettingFocus
        @FocusState.Binding var focusedSetting: SettingFocus?
    ...
    private struct SettingsToggle: View {
        ...
        var body: some View {
            ...
            Button {
                ...
                SettingsRowLabel(label: label, value: L10n.onOff(isOn), isFocused: isFocused)
            }
            .buttonStyle(.plain)
            .focused($focusedSetting, equals: focus)
        }
    ...
    private struct SettingsChoice: View {
        ...
        var body: some View {
            ...
            Button(action: action) {
                SettingsRowLabel(label: label, value: value, isFocused: isFocused)
            }
            .buttonStyle(.plain)
            .focused($focusedSetting, equals: focus)
        }
        ...
        var body: some View {
            HStack {
                ...
                Text(value)
                    ...
                    .foregroundStyle(isFocused ? .black : Color(red: 0.65, green: 0.91, blue: 0.87))
            }
            .padding(.horizontal, 16)
            .padding(.vertical, 12)
            .background(
    ...
    private func sectionHeading(_ title: String) -> some View {
        Text(title)
            ...
            .foregroundStyle(Color(red: 0.65, green: 0.91, blue: 0.87))
            .padding(.bottom, 8)
    }
    ...
    private func labeledField(
        ...
        SettingsField(label: label, placeholder: placeholder, text: text, disableAutocorrect: disableAutocorrect, focus: focus, focusedSetting: $focusedSetting)
    }

    @ViewBuilder
    private func booleanPicker(_ label: String, isOn: Binding<Bool>, focus: SettingFocus) -> SettingsToggle {
        SettingsToggle(label: label, isOn: isOn, focus: focus, focusedSetting: $focusedSetting)
    }

    @ViewBuilder
    private func choiceRow(_ label: String, value: String, focus: SettingFocus, action: @escaping () -> Void) -> SettingsChoice {
        SettingsChoice(label: label, value: value, focus: focus, action: action, focusedSetting: $focusedSetting)
    }
    ...
    @ViewBuilder
    private func actionButton(_ title: String, focus: SettingFocus, disabled: Bool = false, action: @escaping () -> Void) -> SettingsAction {
        SettingsAction(title: title, focus: focus, disabled: disabled, action: action, focusedSetting: $focusedSetting)
    }

    @ViewBuilder
    private func settingRowLabel(_ label: String, value: String, isFocused: Bool) -> SettingsRowLabel {
        SettingsRowLabel(label: label, value: value, isFocused: isFocused)
    }
...
enum SettingsInputValidation {
    ...
    static func validNumber(_ value: String, allowZero: Bool) -> Bool {
